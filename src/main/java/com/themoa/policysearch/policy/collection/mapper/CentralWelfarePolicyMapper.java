...
@Component
public class CentralWelfarePolicyMapper extends PolicyMappingSupport {
    private final XmlMapper xmlMapper = new XmlMapper();
    private final ExternalApiErrorInspector errorInspector;

    public CentralWelfarePolicyMapper(RegionNormalizer regionNormalizer, CategoryNormalizer categoryNormalizer,
    ...
    public CentralWelfareListResponse parseListResponse(int page, ExternalApiResponse response) {
        errorInspector.assertUsable(PolicySource.CENTRAL_WELFARE, page, "XML", response);
        try {
            CentralWelfareListResponse parsed = xmlMapper.readValue(response.body(), CentralWelfareListResponse.class);
            if (parsed.servList() == null) {
                throw new PolicyApiParseException(PolicySource.CENTRAL_WELFARE, page, "XML", response.contentType(),
    ...
    public CentralWelfareDetailResponse parseDetailResponse(String serviceId, ExternalApiResponse response) {
        errorInspector.assertUsable(PolicySource.CENTRAL_WELFARE, 1, "XML", response);
        try {
            return xmlMapper.readValue(response.body(), CentralWelfareDetailResponse.class);
        } catch (Exception ex) {
            throw new PolicyApiParseException(PolicySource.CENTRAL_WELFARE, 1, "XML", response.contentType(),
                    response.body(), response.maskedRequestUrl(), ex);
        }
    }

    public ParsedPolicyPage toPage(CentralWelfareListResponse response) {
    ...
    public PolicyCollectionItem toItem(CentralWelfareListItem listItem, CentralWelfareDetailItem detail) {
        return item(PolicySource.CENTRAL_WELFARE,
                ...
                choose(detail == null ? null : detail.agencyName(), listItem.agencyName()),
                "전국",
                join(listItem.lifeArray(), listItem.lifeNames(), listItem.interestThemeArray(), listItem.interestThemeNames()),
                ...
                detail == null ? null : detail.applicationMethod(),
    ...
    public CentralWelfareDetailItem firstDetail(CentralWelfareDetailResponse response) {
        if (response != null && response.serviceId() != null && !response.serviceId().isBlank()) {
            return new CentralWelfareDetailItem(response.serviceId(), response.serviceName(), response.agencyName(),
    ...
    public List<CentralWelfareListItem> listItems(CentralWelfareListResponse response) {
    ...
    public int totalCount(CentralWelfareListResponse response) {
